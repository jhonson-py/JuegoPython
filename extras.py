...
def dibujar(screen, productos_en_pantalla, producto_principal, producto_candidato, puntos, segundos):
   ...
   # Dibujar los nombres de los productos uno debajo del otro
    ...
    fondo = pygame.image.load("market.jpg").convert() ##Fondo de Img
    ...
    if (segundos<=1):
        gameOverImage = pygame.image.load("gameOver.jpg").convert()
        gameOverR = gameOverImage.get_rect()
        gameOverR.center = (400,300)
        screen.blit(gameOverImage,gameOverR)
        mensaje_puntos = f"Puntos totales: {puntos}"
        ren_puntos = defaultFont.render(mensaje_puntos, 1, COLOR_TEXTO)
        x_puntos = (ANCHO - ren_puntos.get_width()) / 2
        y_puntos = ALTO - ren_puntos.get_height() - 50  # Ajusta la posición verticalmente
        screen.blit(ren_puntos, (x_puntos, y_puntos))
        pygame.display.flip()
